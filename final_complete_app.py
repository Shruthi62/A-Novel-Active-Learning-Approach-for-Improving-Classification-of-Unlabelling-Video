...
st.markdown('<h1 class="main-title">🎥 A Novel Active Learning Approach for Improving Classification of Unlabeled Video Based on Deep Learning Techniques</h1>', unsafe_allow_html=True)
st.markdown('<p class="sub-title">Smart Video Classification with Minimal Labeling Effort</p>', unsafe_allow_html=True)
...
# Sidebar with stats
st.sidebar.markdown('<div class="project-card" style="padding:10px; margin-bottom:10px; border-top: none; border-left: 5px solid #4facfe; background-color: #2d3748;"><h3>📊 Project Dashboard</h3></div>', unsafe_allow_html=True)
st.sidebar.markdown("---")
...
for i, action in enumerate(actions):
    cols[i % 2].markdown(f'<div class="action-caption">• {action}</div>', unsafe_allow_html=True)
...
with tab2:
    ...
    with col2:
        if uploaded_file:
            ...
            st.metric("Object Name", uploaded_file.name[:20] + "...")
...
with tab3:
    ...
    from visualizations import plot_active_learning_progress, plot_labeling_efficiency, plot_uncertainty_distribution
    
    col_v1, col_v2 = st.columns(2)
    with col_v1:
        st.pyplot(plot_active_learning_progress())
    with col_v2:
        st.pyplot(plot_labeling_efficiency())
        
    st.pyplot(plot_uncertainty_distribution())


st.markdown('---')
st.caption('?? Active Learning Video Classification System | Built with Streamlit & PyTorch')
